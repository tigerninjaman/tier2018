		...
		for i,topic in enumerate(topics_list):
			for keyword in topic:
				...
				if all(elem == 0 for elem in r):
					continue
				try:
					q,d,r,p = bd.burst_detection(r,d,n,s=1.5,gamma=0.5,smooth_win=1) # I think the error here is that s = 2 and for 1x2 arrays of r = [1,0] and [1,1] respectively, p[0] = 1/2 so then p=1 (line 60 of burst_detection) which causes an error in line 29 of init in burst_detection. unsure if this is the error since I can't replicate on my console.
				except ValueError:
					r_str = str(r)
					d_str = str(d)
					continue
